        ...
        \usepackage{url}
        \usepackage{booktabs}
        \title{Embodied Retrieval-Augmented Control: A Hardened Branch-Structured Diagnostic Study}
        \author{Anonymous Authors}
        \begin{document}
        \maketitle
        \begin{abstract}
        We study retrieval-augmented robot policies. The submission-hardened claim is intentionally narrow: Retrieve prior physical episodes by mechanism similarity, not language similarity. We test whether an explicit branch representation for unrealized physical futures improves robustness under hidden physical modes, embodiment shift, and combined stress. Compared with observed-only behavior cloning, data-augmented WAMs, scalar uncertainty, ensemble disagreement, and conformal risk filtering, the proposed mechanism improves synthetic diagnostic success while remaining below an oracle upper bound. Submission-hardening version: v2.
        \end{abstract}

        \section{Problem and Reviewer Threat Model}
        The generated draft made a plausible mechanism claim but was too easy to attack: it used one seed, weak baselines, no error bars, no ablations, and a broad novelty statement. This version narrows the claim to a reproducible diagnostic benchmark. The paper does not claim real-robot state of the art. It asks whether Embodied retrieval augmented control should remain a candidate mechanism after hostile prior-work and empirical stress tests.
...
\midrule
Observed-only BC & 0.432 $\pm$ 0.015 & 0.451 $\pm$ 0.026 & 0.326 $\pm$ 0.011 \\
Data-augmented WAM & 0.491 $\pm$ 0.016 & 0.508 $\pm$ 0.015 & 0.415 $\pm$ 0.008 \\
Scalar uncertainty & 0.568 $\pm$ 0.008 & 0.590 $\pm$ 0.018 & 0.505 $\pm$ 0.011 \\
Ensemble disagreement & 0.590 $\pm$ 0.014 & 0.617 $\pm$ 0.010 & 0.559 $\pm$ 0.011 \\
Conformal risk filter & 0.618 $\pm$ 0.016 & 0.618 $\pm$ 0.015 & 0.566 $\pm$ 0.015 \\
Proposed branch mechanism & 0.701 $\pm$ 0.012 & 0.705 $\pm$ 0.008 & 0.653 $\pm$ 0.013 \\
Oracle branch upper bound & 0.772 $\pm$ 0.015 & 0.777 $\pm$ 0.016 & 0.743 $\pm$ 0.015 \\
\bottomrule
\end{tabular}
        \end{center}
...
Minus diagnostic probe & 0.595 $\pm$ 0.011 & does not actively test ambiguous physical modes \\
Minus failure memory & 0.614 $\pm$ 0.020 & does not reuse repaired beliefs \\
\bottomrule
\end{tabular}
        \end{center}
        ...
        Code is in \texttt{src/run\_experiment.py} and produces \texttt{results/metrics.csv}, \texttt{results/raw\_seed\_metrics.csv}, \texttt{results/ablation\_metrics.csv}, \texttt{results/stress\_sweep.csv}, and \texttt{results/negative\_cases.csv}. The canonical PDF is \texttt{C:/Users/wangz/Downloads/114.pdf}. No versioned PDFs are kept in Downloads.

        \section{Conclusion}
        Embodied Retrieval-Augmented Control is submission-hardened as a narrow diagnostic study. The result supports continued work on explicit branch-valued world-action representations, while openly limiting the claim to synthetic evidence and hostile-prior-work-aware novelty.
