...
EXPECTED_HASH = "83CFE32A3356ADD3EF335B5CA4966F972320D65B548118A1BA4C1716A2701412"
...
def main():
    ...
    check(0.40 <= summary["metrics"]["strict_fixed_risk_coverage"] < 1.0, "strict fixed-risk coverage should be meaningful, not a rubber stamp")
    ...
    downloads_pdf = DOWNLOADS / "114.pdf"
    check(paper_pdf.exists(), "paper/main.pdf missing")
    check(downloads_pdf.exists(), "Downloads/114.pdf missing")
    check(not (DESKTOP / "114.pdf").exists(), "Desktop/114.pdf must be absent")
    check(not (ROOT / "114.pdf").exists(), "repo-root 114.pdf must be absent")
    ...
    check(sha256(paper_pdf) == digest, "paper/main.pdf and Downloads/114.pdf differ")
    ...
    print("Paper 114 validation passed.")
